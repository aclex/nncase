...
void onnx_importer::convert_op_Gemm(const NodeProto &node)
{
    ...
    auto alpha_A_op { graph_.emplace<matmul>(alpha_constant->output().shape(), As_shape, bias, value_range<float>::full()) };
	...
	if (node.input().size() > 2)
	{
		...
		auto beta_C_op { graph_.emplace<matmul>(beta_constant->output().shape(), get_shape(input_C), bias, value_range<float>::full()) };
